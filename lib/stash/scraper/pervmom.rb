class Stash::Scraper::Pervmom < Stash::Scraper::SeleniumScraper
  def authenticated?
  ...
  def current_page_url
    return "http://members.pervmom.com/sort/most-recent?page=#{@page}"
  end
  ...
  def scrape
    ...
    loop {
      ...
      elements = @driver.find_elements(xpath: ".//a[contains(@href, 'members.pervmom.com/trailer/')]")
      ...
      items.each { |item|
        try {
          ...
          # video.rating = @driver.find_element(xpath: ".//span[@class='thumbs-percentage']").text.strip
